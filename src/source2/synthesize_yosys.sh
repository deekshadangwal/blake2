...
echo $rootname "is rootname -- pass 2"
#---------------------------------------------------------------------
...
echo "pass 7 -- nested ifs"      
cat > ${rootname}.ys << EOF
# Synthesis script for yosys created by qflow
EOF
echo "synthesis script for yosys created by qflow"
...
#   endif
#endif
echo "pass 13 -- output buffering"
echo $blif_opts
echo "THESE ARE THE BLIF OPTIONS"

cat >> ${rootname}.ys << EOF
# Cleanup
opt
clean
write_blif `echo $blif_opts:s/blake2/ /` ${rootname}_mapped.blif
EOF
